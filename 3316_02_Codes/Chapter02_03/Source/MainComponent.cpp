    ...
    slider1 (Slider::LinearHorizontal, Slider::NoTextBox) // Disable the numerical label on the slider.
{
    ...
    // Make our label editible on a single click.
    label1.setEditable (true);
    
    // Add this class as a listener to the button, slider and label.
...
{
    // Check if this was our label that changed and, if so, update the slider.
    if (&label1 == label) {
